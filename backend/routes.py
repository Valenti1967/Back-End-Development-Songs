...
def songs():
    ...
    json_data = json_util.dumps({"songs": results})
...
def create_song():
    ...
    results = db.songs.find_one({"id":new_song["id"]})
    results_list = list(results)
    if len(results_list) < 1:
        result_insert = db.songs.insert_one(new_song) 
        response = {"inserted id": {"$oid": str(result_insert.inserted_id)}}
